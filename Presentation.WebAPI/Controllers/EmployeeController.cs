...
namespace Presentation.WebAPI.Controllers
{
    ...
    public class EmployeeController : BaseAPIController
    {
        ...
        public async Task<string> UploadImage([FromForm] UploadImage imageFile)
        {
            var routeImage = String.Empty;

            if(imageFile.fileImage.Length > 0)
            {
                var nameFile = Guid.NewGuid().ToString() +".jpg";
                routeImage = $"../Domain.Model/StorageImage/{nameFile}";
                using (var stream = new FileStream(routeImage, FileMode.Create))
                {
                    await imageFile.fileImage.CopyToAsync(stream);
                }
            }
